...
class Inviters extends Component {
  ...
  componentWillMount() {
    ...
    this.props.dispatch(Actions.GetInviters(token))
  }

  render() {
    const { inviters } = this.props

    return (
      <View style={styles.container}>
        <View style={[styles.header, styles.row]}>
          <View style={styles.coverTitle}><Text style={styles.title}> Inviters</Text></View>
          <Button style={styles.btnIcon}><Icon name='edit' color={Color.white} size={20}/></Button>
        </View>
        <View style={styles.component}>
          <Lists dataSource={inviters} />
        </View>
      </View>
    );
  }
...
export default connect(map)(Inviters)
